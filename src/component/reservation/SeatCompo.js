...
function SeatCompo() {
  ...
  function goProPage(seatId) {
    navigate(`/pro/${seatId}/${params.mid}?k=1`);
  }
  return (
    <div>
      {
      ...
      {state.ReserReducers.seatInfoArr.map(info => {
        return <img style={{ left: info.left + 'rem', top: info.top + 'rem', cursor: 'pointer',opacity: info.soldOut === 0 ? 0.1 : 1 }} onClick={() => { if(info.soldOut===0){alert('예약 할 수 없는 좌석입니다'); return;} goProPage(info.id) }} onMouseEnter={(event) => zoomIn(event)} onMouseLeave={(event) => { zoomOut(event) }} key={info.id} className="seat" src={info.url} alt="seat" />
      })}
    </div>
  );
}
